...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `calendar/` — Multi-provider calendar abstraction. `providerFactory.ts` returns a `CalendarProvider` (`getCalendarProvider(accountId)`) based on `account.provider`/`account.calendar_provider`: `googleCalendarProvider.ts` (Gmail accounts, full read/write via Calendar API), `caldavProvider.ts` (standalone or IMAP-linked CalDAV accounts, full read/write via `tsdav`), `icsUrlProvider.ts` (read-only `.ics`/`webcal` feed subscriptions, one-way sync, `createEvent`/`updateEvent`/`deleteEvent` all throw). `icalHelper.ts` generates/parses VEVENT blocks (`parseICSFeed` splits a multi-event feed). `autoDiscovery.ts` detects CalDAV server settings from an email domain. `calendarActions.ts`'s `createAndCacheCalendarEvent()` creates an event via the provider and immediately upserts it into the local cache — the single write path shared by the Calendar page's create-event flow and the Telegram assistant's `propose_event` tool. CalDAV and ICS requests go through `@tauri-apps/plugin-http`'s `fetch` (not the native browser `fetch`) since arbitrary user-supplied server hosts can't be enumerated in the CSP `connect-src` allowlist — native fetch would fail with a WebKit `TypeError: Load failed`.
   - `assistant/` — Telegram-based phone assistant. `assistantManager.ts` long-polls Telegram (`telegram.ts`, a minimal fetch-based Bot API client) while Velo is running, enforces a single allow-listed Telegram user ID, and exposes live status (`onAssistantStatus`) for the Settings UI. `agent.ts` runs a Claude tool-use loop (`runAgentTurn`) seeded with the current date/time so it can resolve relative dates. `tools.ts` defines the agent's tools: `list_recent_threads`/`read_thread` (local-cache email reads), `propose_reply` (stages a reply), `list_calendars`/`list_calendar_events` (local-cache calendar reads across all accounts), `propose_event` (stages a new event). The two `propose_*` tools never write directly — they call `ctx.stageReply`/`ctx.stageEvent` to hand a draft back to `assistantManager.ts`, which presents it in Telegram with confirmation buttons (Send/Save/Cancel or Add/Cancel) and only writes on an explicit tap.
   - `google/` — `calendar.ts` handles Google Calendar API (list calendars, fetch events, create events, token refresh); used internally by `calendar/googleCalendarProvider.ts`.
...
- `settings/` — SettingsPage, FilterEditor, LabelEditor, SignatureEditor, TemplateEditor, ContactEditor, SubscriptionManager, QuickStepEditor, SmartFolderEditor, ImapAccountSettings, AssistantSettings
- `accounts/` — AddAccount, AddImapAccount, AddCalDavAccount, AddIcsCalendarAccount, AccountSwitcher, SetupClientId
...
137 test files across stores (8), services (74), utils (15), components (33), constants (3), router (1), hooks (2), and config (1).
...
- **Calendar-only accounts**: `account.provider` of `"caldav"` or `"ics_url"` marks a standalone calendar account with no email — `syncManager.ts` routes these to calendar-only sync and skips Gmail/IMAP sync entirely; `App.tsx`'s send-as-alias and Gmail-client-init logic excludes both. ICS accounts have no real email, so a placeholder (`ics-{id}@velo.local`) satisfies the `accounts.email NOT NULL UNIQUE` constraint — never shown in the UI, which always prefers `display_name` for this provider
- **Phone assistant writes**: Both agent tools that mutate anything (`propose_reply`, `propose_event`) only *stage* a draft via `AssistantContext.stageReply`/`stageEvent` — they never send/create directly. `assistantManager.ts` presents the draft in Telegram with confirmation buttons; the actual write happens only on a button tap, in `handleCallback`. Keep this pattern for any new agent write tool — the system prompt explicitly tells the model never to claim something was sent/created before confirmation
- **Frontend logging visibility**: `console.log` in frontend code only reaches the WebView devtools console, never the terminal running `npm run tauri dev` — they're separate log sinks. To have a message show up in the terminal, use `@tauri-apps/plugin-log`'s `info`/`warn`/`error` (forwards via IPC to the Rust `log` crate, which is configured with the default Stdout target in `src-tauri/src/lib.rs`). Requires the `log:default` capability permission. `assistantManager.ts`'s `log()`/`logWarn()`/`logErr()` wrappers do both at once
